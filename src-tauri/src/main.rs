...
fn main() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![greet, create_ttmp])
...
async fn create_ttmp<R: Runtime>(window: Window<R>, path: &str, info: ModInfo, groups: Vec<Group>, needed_files: NeededFiles) -> Result<(), String> {
    create_ttmp_inner(window, path, info, groups, needed_files).await
        .map_err(|e| format!("{:#}\n{}", e, e.backtrace()))
}

async fn create_ttmp_inner<R: Runtime>(window: Window<R>, path: &str, info: ModInfo, groups: Vec<Group>, needed_files: NeededFiles) -> anyhow::Result<()> {
    TtmpProgress::CreatingManifest.emit(&window)?;

    // create two temp files (staging file and mpd)
    // download each file into the staging file
    // add each file into the mpd (maybe don't need staging file?)
    // convert v1 manifest into v2 if necessary
    // create the ttmp zip archive
    // add mpl and mpd
    // return success
    ...
    for uses in needed_files.files.values() {
        for use_ in uses {
            ...
            let option = use_.0.as_ref().unwrap();
